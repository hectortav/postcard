 ...
 * {@link SendmeSession} reference itself, so {@link #remove(Object)} is identity-based
 ...
 *
 * <p>Task 2.10 will wire the real Jetty/WebSocket adapter to {@link SendmeSession};
...
public final class WebSocketHub {
    ...
    private final ConcurrentHashMap<SendmeSession, Boolean> sessions = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<SendmeSession, LinkedBlockingQueue<String>> queues = new ConcurrentHashMap<>();
    private final ExecutorService pool = Executors.newVirtualThreadPerTaskExecutor();

    public void add(SendmeSession s) {
        if (s == null) return;
        queues.put(s, new LinkedBlockingQueue<>(QUEUE_CAPACITY));
        sessions.put(s, Boolean.TRUE);
    }

    public void remove(SendmeSession s) {
        if (s == null) return;
        sessions.remove(s);
        queues.remove(s);
    }
