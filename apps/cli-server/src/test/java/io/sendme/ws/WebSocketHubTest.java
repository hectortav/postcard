...
class WebSocketHubTest {
    static class FakeSession implements SendmeSession {
        final String key;
    ...
    @Test void removeIsIdentityBased() {
        ...
        // Adding a fresh anonymous SendmeSession and trying to remove it must be a no-op
        hub.add(a);
        hub.remove(new SendmeSession() {
