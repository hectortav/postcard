...
// `&pin=5678`. The fragment is the client-side signal that gates the
// PinLockScreen on mount; the actual unlock is POST /api/pin/verify,
// where the server re-derives the AES key from (secret, pin, salt) and
// compares it to its stored expected key. The fragment is never sent to
// the server, so we navigate the browser to the absolute URL with the
// fragment (Playwright strips the fragment when resolving a relative
// `goto('/')` against baseURL).
...
test('correct PIN unlocks the dashboard and files round-trip', async ({ browser }) => {
  ...
  // 200 → onVerified → App flips pinUnlocked → dashboard renders.
  await expect(host.getByText(/no files yet/i)).toBeVisible({ timeout: 10_000 });
  ...
  await expect(mobile.getByText(/no files yet/i)).toBeVisible({ timeout: 10_000 });
