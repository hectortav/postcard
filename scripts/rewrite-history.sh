#!/usr/bin/env bash
# rewrite-history.sh — neutral git history cleanup for the postcard repo.
#
...
mkdir -p .git/postcard-rewrite
> .git/postcard-rewrite/timestamps.env
echo "# auto-generated by scripts/rewrite-history.sh" >> .git/postcard-rewrite/timestamps.env
echo "# source this in your rebase environment to override dates" >> .git/postcard-rewrite/timestamps.env
...
Next step — start an interactive rebase using these timestamps:
  ...
  GIT_SEQUENCE_EDITOR="$(pwd)/.git/postcard-rewrite/sequence-editor.sh" \
      git rebase -i --root
